...
2. As User A, add one identifier, event, match, archive file, email import, email finding, saved-logins import, and saved-logins finding.
...
8. Repeat the direct-ID attempts against User A's `email_imports`, `email_findings`, `login_exports`, and `login_export_findings` rows, including an attempt to link User B's finding to User A's timeline event. Every operation must fail or return no accessible rows.
...
- Deletion: individual records, email-import cascade, login-export cascade, delete-all history after export, profile removal, failed Storage cleanup aborting database deletion, Edge Function account deletion
