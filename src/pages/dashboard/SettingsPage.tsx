...
const EXPORT_TABLES = ['profiles', 'identifiers', 'timeline_events', 'possible_matches', 'email_imports', 'email_findings', 'login_exports', 'login_export_findings', 'archive_files', 'event_files', 'user_consents', 'activity_log'] as const

export function SettingsPage() {
  const { user, updatePassword, signOut } = useAuth()
  const { plan, billing, openPortal } = useBilling()
  ...
  return (
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <section id="billing" className="scroll-mt-8 border border-ink/10 bg-white p-7 xl:col-span-2"><p className="text-label uppercase text-gold">Billing</p><h2 className="mt-2 text-2xl font-semibold">{plan === 'vault' ? 'Vault plan' : plan === 'recovery' ? 'Recovery unlocked' : 'Choose your recovery plan'}</h2><p className="mt-2 max-w-2xl text-body-s text-ink/55">{plan === 'vault' ? `Your cumulative Vault access is active${billing?.cancel_at_period_end && billing.current_period_end ? ` until ${new Date(billing.current_period_end).toLocaleDateString()}` : ''}.` : plan === 'recovery' ? 'Your one-time Recovery purchase remains unlocked permanently. Upgrade to Vault whenever you want expanded private storage.' : 'Recovery unlocks complete recovery tools once. Vault includes Recovery plus expanded private storage.'}</p>{plan !== 'vault' && <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="border border-ink/10 p-5"><p className="font-semibold">Recovery</p><p className="mt-1 text-body-s text-ink/55">Full recovery tools and exports. Pay once.</p><p className="mt-4 text-xl font-semibold">$19.99 <span className="text-body-s font-normal text-ink/50">one time</span></p>{plan === 'free' ? <PlanButton plan="recovery" className={`${secondaryButtonClass} mt-5`}>Unlock Recovery</PlanButton> : <p className="mt-5 text-body-s font-medium text-emerald-700">Included in your account</p>}</div><div className="border border-gold/50 bg-[#fffaf0] p-5"><p className="font-semibold">Vault</p><p className="mt-1 text-body-s text-ink/55">Recovery, plus expanded private storage.</p><p className="mt-4 text-xl font-semibold">$7.99 <span className="text-body-s font-normal text-ink/50">per month</span></p><PlanButton plan="vault" className={`${primaryButtonClass} mt-5`}>Get Vault</PlanButton></div></div>}{billing?.stripe_customer_id && <button type="button" onClick={() => void billingAction()} disabled={working} className={`${secondaryButtonClass} mt-5`}><CreditCard size={16} className="mr-2" />Manage billing</button>}</section>
