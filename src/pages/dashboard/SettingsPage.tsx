import { Download, KeyRound, Save, ShieldAlert, Trash2 } from 'lucide-react'
...
export function SettingsPage() {
  ...
  const deleteApplicationData = async () => {
    if (!supabase || !user) return
    setWorking(true)
    const { data: files } = await supabase.from('archive_files').select('storage_path')
    const paths = (files ?? []).map((file) => file.storage_path as string)
    if (paths.length) await supabase.storage.from('private-archives').remove(paths)
    const { error } = await supabase.rpc('delete_my_application_data')
    setWorking(false)
    if (error) toast.error('Your data could not be fully deleted. No partial success is being claimed.')
  ...
  return (
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="border border-ink/10 bg-white p-7"><p className="text-label uppercase text-gold">Profile</p><h2 className="mt-2 text-2xl font-semibold">Account identity</h2><div className="mt-5 border border-emerald-200 bg-emerald-50 p-4"><p className="text-micro uppercase text-emerald-700">Verified account email</p><p className="mt-1 font-medium text-emerald-950">{user?.email}</p><p className="mt-1 text-micro text-emerald-800">This label applies only to your authenticated account email—not historical emails you add.</p></div><form onSubmit={saveProfile} className="mt-5 space-y-5"><Field label="Display name" htmlFor="display-name"><input id="display-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} className={inputClass} /></Field><button type="submit" disabled={saving} className={primaryButtonClass}><Save size={16} className="mr-2" />{saving ? 'Saving…' : 'Save profile'}</button></form></section>
