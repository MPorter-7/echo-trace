...
export function IdentifiersPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="Identity clues" title="My identifiers" description="Add usernames, historical emails, and public URLs that belong to you. Historical emails are never marked verified unless they are your authenticated account email." action={<button type="button" onClick={openCreate} className={primaryButtonClass}><Plus size={17} className="mr-2" />Add identifier</button>} />
      ...
      ) : <EmptyState title={search ? 'No identifiers match' : 'Add a clue that belongs to you'} description={search ? 'Try a different search term.' : 'A former username, profile URL, or historical email is a strong place to begin.'} action={!search && <button type="button" onClick={openCreate} className={primaryButtonClass}>Add identifier</button>} />}
